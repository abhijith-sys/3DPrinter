...
function FeatureIcon({ variant }: { variant: 1 | 2 | 3 }) {
  ...
  if (variant === 1) {
    return (
      <svg width="28" height="28" viewBox="0 0 28 28" fill="none" aria-hidden className="text-on-surface">
        <rect x="3" y="3" width="22" height="22" rx="4" stroke={stroke} strokeWidth="1.5" />
        <circle cx="9" cy="9" r="1.5" fill={stroke} />
        <circle cx="19" cy="9" r="1.5" fill={stroke} />
        <circle cx="9" cy="19" r="1.5" fill={stroke} />
        <circle cx="19" cy="19" r="1.5" fill={stroke} />
        <path d="M9 9h10M9 9v10M19 9v10M9 19h10" stroke={stroke} strokeWidth="1.2" />
      </svg>
    )
  }
  if (variant === 2) {
    return (
      <svg width="28" height="28" viewBox="0 0 28 28" fill="none" aria-hidden className="text-on-surface">
        <rect x="3" y="3" width="22" height="22" rx="4" stroke={stroke} strokeWidth="1.5" />
        <path d="M7 20L21 8" stroke={stroke} strokeWidth="1.5" />
        <circle cx="7" cy="20" r="2" fill={stroke} />
        <circle cx="14" cy="14" r="2" fill={stroke} />
        <circle cx="21" cy="8" r="2" fill={stroke} />
      </svg>
    )
  }
  return (
    <svg width="28" height="28" viewBox="0 0 28 28" fill="none" aria-hidden className="text-on-surface">
      <rect x="3" y="3" width="22" height="22" rx="4" stroke={stroke} strokeWidth="1.5" />
      <circle cx="14" cy="10" r="2" fill={stroke} />
      <circle cx="9" cy="18" r="2" fill={stroke} />
      <circle cx="19" cy="18" r="2" fill={stroke} />
      <path d="M14 12v3M14 15l-4 2M14 15l4 2" stroke={stroke} strokeWidth="1.3" />
    </svg>
  )
}
...
export default function Landing() {
  ...
  return (
    <div className="bg-page text-on-surface min-h-screen flex flex-col selection:bg-secondary-container relative">
        ...
        <section className="flex-grow grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-8 px-6 md:px-12 lg:px-16 pt-4 lg:pt-8 pb-8 items-center">
          <div className="max-w-[540px]">
            <h1 className="font-headline-xl text-[36px] sm:text-[44px] md:text-[52px] leading-[1.15] tracking-[-0.03em] font-bold text-on-surface">
              ...
              <JoyGlyph />
              of{' '}
              <span className="inline bg-secondary-container text-[#1c1b1b] rounded-full px-[0.35em] py-[0.05em] box-decoration-clone">
                effortless
              </span>{' '}
            ...
            <p className="mt-8 text-muted text-[15px] leading-[1.65] max-w-[380px]">
        ...
        >
          ...
          {features.map((f, i) => (
            <Link
              key={f.title}
              to={f.to}
              className={`px-6 md:px-8 py-8 lg:py-12 flex flex-col gap-3 hover:bg-surface-container-low transition-colors border-hairline ${
            ...
            >
              ...
              <p className="text-[12px] md:text-[13px] text-muted leading-snug">{f.body}</p>
            </Link>
          ))}
        </section>
      </main>
